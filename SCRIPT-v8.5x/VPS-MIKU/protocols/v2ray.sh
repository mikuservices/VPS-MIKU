...
err_fun () {
     ...
     2)msg -verm "$(fun_trans "User muy corto (MIN: 2 CHARACTERS)")"; sleep 2s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
     3)msg -verm "$(fun_trans "User muy largo (MAX: 5 CHARACTERS)")"; sleep 2s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
     ...
     8)msg -verm "$(fun_trans "Duracion invalida. Solo numeros")"; sleep 2s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
     9)msg -verm "$(fun_trans "Maxima duracion de 12 meses")"; sleep 2s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
     11)msg -verm "$(fun_trans "Null Limit")"; sleep 2s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
     12)msg -verm "$(fun_trans "Duracion invalida. Usa numeros")"; sleep 2s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
     13)msg -verm "$(fun_trans "Limite maximo de 999")"; sleep 2s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
     14)msg -verm "$(fun_trans "Este usuario ya existe.")"; sleep 2s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
	 ...
	 17)msg -verm "$(fun_trans "(Sin Informacion - Para cancelar presiona CTRL + C)")"; sleep 4s; tput cuu1; tput dl1; tput cuu1; tput dl1;;
     esac
}
intallv2ray () {
...
USRdatabase="/etc/VPS-MIKU/RegV2ray"
...
protocolv2ray () {
msg -ama "$(fun_trans "Elige opcion 3 e inserta el dominio de tu IP")!"
msg -bar
v2ray stream
msg -bar
msg -ne "Pulsa enter para continuar" && read enter
...
portv () {
msg -ama "$(fun_trans "Cambiar puerto de V2RAY")!"
msg -bar
v2ray port
msg -bar
msg -ne "Pulsa enter para continuar" && read enter
...
rm -rf /etc/VPS-MIKU/RegV2ray > /dev/null 2>&1
echo -e "\033[1;92m                  V2RAY ELIMINADO EXITOSAMENTE "
msg -bar
msg -ne "Pulsa enter para continuar" && read enter
${SCPinst}/v2ray.sh
}
...
msg -bar
msg -tit
msg -ama "             AGREGAR USER | UUID V2RAY"
msg -bar
##DAIS
...
echo ""
while true; do
echo -ne "\e[91m >> Introduce un nombre: \033[1;92m"
     read -p ": " nick
...
echo -e "\e[91m >> UUID Agregado es: \e[92m$UUID "
while true; do
...
##Registro
echo "  $UUID | $nick | $valid " >> /etc/VPS-MIKU/RegV2ray
Fecha=`date +%d-%m-%y-%R`
cp /etc/VPS-MIKU/RegV2ray /etc/VPS-MIKU/v2ray/RegV2ray-"$Fecha"
cp /etc/v2ray/config.json /etc/VPS-MIKU/v2ray/config.json-"$Fecha" 
v2ray restart > /dev/null 2>&1
echo ""
v2ray info > /etc/VPS-MIKU/v2ray/confuuid.log
lineP=$(sed -n '/'${UUID}'/=' /etc/VPS-MIKU/v2ray/confuuid.log)
numl1=4
let suma=$lineP+$numl1
sed -n ${suma}p /etc/VPS-MIKU/v2ray/confuuid.log 
echo ""
msg -bar
echo -e "\e[92m           UUID AGREGADO EXITOSAMENTE "
msg -bar
msg -ne "Pulsa enter para continuar" && read enter
${SCPinst}/v2ray.sh
}
...
msg -bar
msg -tit
msg -ama "             ELIMINAR USERS | UUID V2RAY"
msg -bar
echo -e "\e[97m               USERS REGISTRADOS"
echo -e "\e[33m$(cat /etc/VPS-MIKU/RegV2ray|cut -d '|' -f2,1)" 
msg -bar
echo -ne "\e[91m >> Introduce el UUID a eliminar:\n \033[1;92m " && read uuidel
[[ $(sed -n '/'${uuidel}'/=' /etc/v2ray/config.json|head -1) ]] || invaliduuid
lineP=$(sed -n '/'${uuidel}'/=' /etc/v2ray/config.json)
linePre=$(sed -n '/'${uuidel}'/=' /etc/VPS-MIKU/RegV2ray)
...
msg -bar
msg -tit
msg -ama "         USERS REGISTRADOS | UUID V2RAY"
msg -bar
# usersss=$(cat /etc/VPS-MIKU/RegV2ray|cut -d '|' -f1)
# cat /etc/VPS-MIKU/RegV2ray|cut -d'|' -f3
VPSsec=$(date +%s)
...
usris="$(cat /etc/VPS-MIKU/RegV2ray|grep -w "$hostreturn"|cut -d'|' -f2)"
...
msg -bar
msg -tit
msg -ama "          LIMITE MB X PUERTO | UUID V2RAY"
msg -bar
###VER
estarts () {
VPSsec=$(date +%s)
local HOST="/etc/VPS-MIKU/v2ray/lisportt.log"
local HOST2="/etc/VPS-MIKU/v2ray/lisportt.log"
local RETURN="$(cat $HOST|cut -d'|' -f2)"
local IDEUUID="$(cat $HOST|cut -d'|' -f1)"
if [[ -z $RETURN ]]; then
echo -e "----- NO HAY REGISTROS -----"
...
i=1
while read hostreturn ; do
iptables -n -v -L > /etc/VPS-MIKU/v2ray/data1.log 
statsss=$(cat /etc/VPS-MIKU/v2ray/data1.log|grep -w "tcp spt:$hostreturn quota:"|cut -d' ' -f3,4,5)
gblim=$(cat /etc/VPS-MIKU/v2ray/lisportt.log|grep -w "$hostreturn"|cut -d'|' -f2)
...
[[ ! -z $contador_secuencial ]] && {
linesss=$(cat /etc/VPS-MIKU/v2ray/lisportt.log | wc -l)
	      echo -e "$contador_secuencial \n Limited Ports: $linesss"
	}
fi
msg -bar
msg -ne "Pulsa ennter para continuar" && read enter
${SCPinst}/v2ray.sh 
}
###LIM
liport () {
while true; do
     echo -ne "\e[91m >> Presiona el puerto a limitar:\033[1;92m " && read portbg
     if [[ -z "$portbg" ]]; then
     err_fun 17 && continue
...
while true; do
     echo -ne "\e[91m >> Cantidad de GB de tráfico:\033[1;92m " && read capgb
     if [[ -z "$capgb" ]]; then
     err_fun 17 && continue
     elif [[ "$capgb" != +([0-9]) ]]; then
     err_fun 15 && continue
...
echo ""
echo " $portbg | $gbuser | $multiplicacion " >> /etc/VPS-MIKU/v2ray/lisportt.log 
msg -bar
msg -ne "Pulsa enter para continuar" && read enter
${SCPinst}/v2ray.sh
}
...
while true; do
     echo -ne "\e[91m >> Puerto para eliminar el limite:\033[1;92m " && read portbg
     if [[ -z "$portbg" ]]; then
     err_fun 17 && continue
     elif [[ "$portbg" != +([0-9]) ]]; then
     err_fun 16 && continue
...
[[ $(sed -n '/'${portbg}'/=' /etc/VPS-MIKU/v2ray/lisportt.log|head -1) ]] || invaliduuid
gblim=$(cat /etc/VPS-MIKU/v2ray/lisportt.log|grep -w "$portbg"|cut -d'|' -f3)
sudo iptables -D OUTPUT -p tcp --sport $portbg -j DROP
sudo iptables -D OUTPUT -p tcp --sport $portbg -m quota --quota $gblim -j ACCEPT
iptables-save > /etc/iptables/rules.v4
lineP=$(sed -n '/'${portbg}'/=' /etc/VPS-MIKU/v2ray/lisportt.log)
sed -i "${linePre}d" /etc/VPS-MIKU/v2ray/lisportt.log
msg -bar
msg -ne "Pulsa enter para continuar" && read enter
${SCPinst}/v2ray.sh 
}
## MENU
echo -ne "\033[1;32m [1] > " && msg -azu "$(fun_trans "LIMITE DE TRAFICO POR PUERTO") "
echo -ne "\033[1;32m [2] > " && msg -azu "$(fun_trans "REINICIAR EL TRAFICO DEL PUERTO") "
echo -ne "\033[1;32m [3] > " && msg -azu "$(fun_trans "VER EL CONSUMO DE TRAFICO") "
...
echo -ne "\033[1;37m$(fun_trans " ► Selecciona una opcion"): " >&2
read selection
...
msg -bar
msg -tit
msg -ama "$(fun_trans "        INSTALADOR V2RAY (PASO A PASO) ")"
msg -bar
## INSTALADOR
...
## CONTROLER
echo -ne "\033[1;32m [5] > " && msg -azu "AGREGAR USER UUID "
echo -ne "\033[1;32m [6] > " && msg -azu "ELIMINAR USER UUID"
echo -ne "\033[1;32m [7] > " && msg -azu "MOSTRAR LOS USUARIOS REGISTRADOS"
echo -ne "\033[1;32m [8] > " && msg -azu "INFORMACION DE LAS CUENTAS"
echo -ne "\033[1;32m [9] > " && msg -azu "CONSUMO DEL TRAFICO "
...
echo -e "         \e[97mEstado: $(pid_inst v2ray)"
msg -bar
