...
check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error} El user no es ROOT (no tienes permiso ROOT), no podemos continuar con la operacion, por favor introduce ${Green_background_prefix} sudo su ${Font_color_suffix} para continuar (Puede pedirte tu contraseña root)" && exit 1
}
check_sys(){
...
check_crontab(){
	[[ ! -e "/usr/bin/crontab" ]] && echo -e "${Error}Crontab dependency no fue encontrada, Trata de instalar manualmente CentOS: yum install crond -y , Debian/Ubuntu: apt-get install cron -y !" && exit 1
}
SSR_installation_status(){
	[[ ! -e ${ssr_folder} ]] && echo -e "${Error}\nShadowsocksR Folder no encontrado, por favor revisa.\n$(msg -bar)" && exit 1
}
Server_Speeder_installation_status(){
	[[ ! -e ${Server_Speeder_file} ]] && echo -e "${Error}No instalado (Server Speeder), por favor revisa!" && exit 1
}
LotServer_installation_status(){
	[[ ! -e ${LotServer_file} ]] && echo -e "${Error}LotServer no instalado, por favor revisa!" && exit 1
}
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		echo -e "${Error} No encontramos el script BBR, iniciando descarga ..."
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/bbr.sh; then
			...
			echo -e "${Info} BBR script fue descargado con exito!"
			chmod +x bbr.sh
		fi
	fi
}
...
clear
	...
	while true
	do
		echo -e "Introduce el usuario para revisar su\ninformacion"
msg -bar
		...
		if [[ ! -z ${View_user} ]]; then
			...
			echo -e "${Error} Por favor introduce el puerto correcto!"
		fi
	done
...
msg -bar
    echo -e " PANEL VPS-MIKU By @mikuservices"
...
msg -bar
	stty erase '^H' && read -p "(Default: VPS-MIKU):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="VPS-MIKU"
...
msg -bar
	while true
	do
	echo -e "\e[92m 2) Por favor introduce un puerto para el usuario "
msg -bar
	...
	if [[ $? == 0 ]]; then
		if [[ ${ssr_port} -ge 1 ]] && [[ ${ssr_port} -le 65535 ]]; then
			...
			echo -e "${Error} Por favor introduce un numero valido (1-65535)"
		fi
	else
		echo -e "${Error} Por favor introduce un numero valido (1-65535)"
	fi
...
Set_config_password(){
msg -bar
	echo -e "\e[92m 3) Por favor introduce una contrasena para el usuario"
msg -bar
	stty erase '^H' && read -p "(Default: VPS-MIKU):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="VPS-MIKU"
	echo && echo -e "	Password : ${Green_font_prefix}${ssr_password}${Font_color_suffix}" && echo
}
Set_config_method(){
msg -bar
	echo -e "\e[92m 4) Selecciona un tipo de encriptado\e[0m
$(msg -bar)
...
Set_config_protocol(){
msg -bar
	echo -e "\e[92m 5) Por favor selecciona un protocolo
$(msg -bar)
...
msg -bar
	if [[ -e ${ssr_folder} ]]; then
		check_pid
		if [[ ! -z "${PID}" ]]; then
			 echo -e "         VPS-MIKU By @mikuservices\n Estado actual: ${Green_font_prefix}instalado${Font_color_suffix} y ${Green_font_prefix}funcionando${Font_color_suffix}"
		else
			echo -e " Estado actual ${Green_font_prefix}instalado${Font_color_suffix} pero ${Red_font_prefix}no esta operativo${Font_color_suffix}"
		fi
		cd "${ssr_folder}"
	else
...
check_sys
[[ ${release} != "debian" ]] && [[ ${release} != "ubuntu" ]] && [[ ${release} != "centos" ]] && echo -e "${Error} El script no es compatible con este sistema ${release} !" && exit 1
action=$1
