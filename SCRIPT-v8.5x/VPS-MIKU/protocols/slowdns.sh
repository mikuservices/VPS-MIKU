#!/bin/bash
 #
 ADM_inst="/etc/VPS-MIKU/Slow/install" && [[ ! -d ${ADM_inst} ]] && exit
 ADM_slow="/etc/VPS-MIKU/Slow/Key" && [[ ! -d ${ADM_slow} ]] && exit
 info(){
 	clear
 	nodata(){
 		msg -bar
 	...
 	msg -bar
 	msg -ama "Tu public key: $(cat ${ADM_slow}/server.pub)"
 ...
 ini_slow(){
     ...
     echo -e " $(msg -ama "Elige el puerto para SLOWDNS:") $(msg -verd "$PORT")"
     msg -bar
 
     unset NS
     while [[ -z $NS ]]; do
     	msg -ama "Tu dominio NS (Nameserver): "
     	read NS
     	tput cuu1 && tput dl1
     done
     echo "$NS" > ${ADM_slow}/domain_ns
     echo -e " $(msg -ama "Tu dominio NS (Nameserver)") $(msg -verd "$NS")"
     msg -bar
 
     if [[ ! -e ${ADM_inst}/dns-server ]]; then
     	msg -ama " Downloading binary...."
     	if wget -O ${ADM_inst}/dns-server https://raw.githubusercontent.com/mikuservices/VPS-MIKU/master/LINKS-LIBRARIES/dns-server &>/dev/null ; then
     		chmod +x ${ADM_inst}/dns-server
     		msg -verd "[OK]"
     	else
     		msg -verm "[fail]"
     		msg -bar
     		msg -ama "No es posible descargar el binario"
     ...
     if [[ ! -z "$pub" ]]; then
     	msg -ama " Usar una key existente? [Y/N]: "
     	read ex_key
