...
BadVPN() {
	...
	if [[ ! -e /bin/badvpn-udpgw ]]; then
		wget -O /bin/badvpn-udpgw https://raw.githubusercontent.com/mikuservices/VPS-MIKU/master/LINKS-LIBRARIES/badvpn-udpgw &>/dev/null
		chmod 777 /bin/badvpn-udpgw
	fi
	#fix rclocal
	msg -bar
	msg -tit
	msg -ama "  \e[1;43m\e[91mACTIVADOR BADVPN (7100-7200-7300-Multi Port)\e[0m"
	msg -bar
	echo -e "$(msg -verd "[1]")$(msg -verm2 "➛ ")$(msg -azu "ACTIVAR BADVPN 7300")"
	echo -e "$(msg -verd "[2]")$(msg -verm2 "➛ ")$(msg -azu "AGREGAR +PORT BADVPN ")"
	echo -e "$(msg -verd "[3]")$(msg -verm2 "➛ ")$(msg -azu "DETENER BADVPN")"

	msg -bar
	read -p "Elige una opcion (default 1): " -e -i 1 portasx
	tput cuu1 && tput dl1
	if [[ ${portasx} = 1 ]]; then
		...
		[[ ! "$(ps x | grep badvpn | grep -v grep | awk '{print $1}')" ]] && msg -ne "                DESACTIVADO CORRECTAMENTE \n"
		unset pid_badvpn
		msg -bar
	elif [[ ${portasx} = 0 ]]; then
		msg -verm "	Exiting"
