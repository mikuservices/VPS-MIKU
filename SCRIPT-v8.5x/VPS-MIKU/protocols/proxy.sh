#!/bin/bash
 ...
 SCPinst="/etc/VPS-MIKU/protocols"
 mportas () {
 unset portas
 ...
 det_py(){
 	...
 	rm /etc/VPS-MIKU/PortPD.log &>/dev/null
 	rm -rf /etc/VPS-MIKU/protocols/lacasitamx.py
 ...
 clear
 echo ""
 if [[ ! -e /etc/VPS-MIKU/fix ]]; then
 		echo ""
 ins(){
 apt-get install python -y 
 apt-get install python-pip -y
 }
 ins &>/dev/null && echo -e "INSTALANDO ARREGLOS" | pv -qL 40
 sleep 1.s
 [[ ! -e /etc/VPS-AGN/fix ]] && touch /etc/VPS-MIKU/fix
 else
 echo ""
 fi
 clear
 ...
 while true; do
 	echo -ne "\033[1;37m"
     read -p " ESCRIBE TU PUERTO: " proxycolor
 	echo -e ""
    ...
    echo -e "$BARRA" 
     while true; do
          ...
          echo -ne "\033[1;36m"
          read -p " INTRODUCE EL PUERTO LOCAL (22|443|80): " PORTLOC
 		 echo ""
          if [[ ! -z $PORTLOC ]]; then
 ...
 echo -e "$BARRA"
 read -p " CUAL SERA LA HTTP RESPONSE? (101|200|300): " encabezad
 tput cuu1 && tput dl1
      if [[ -z $encabezad ]]; then
         encabezad="200"
 ...
 echo -e "$BARRA"
 echo -ne " Escribe tu minibanner en HTML:\n \033[1;37m" && read mensage
 tput cuu1 && tput dl1
      if [[ -z $mensage ]]; then
         mensage="@lacasitamx"
     ...
     echo -e "$BARRA"
     [[ ! -e /etc/VPS-MIKU/protocols/lacasitamx.py ]] && rm -rf /etc/VPS-MIKU/protocols/lacasitamx.py
 #
 	# Initializing or Proxy
 	(
 	less << CPM > /etc/VPS-MIKU/protocols/lacasitamx.py
 # -*- coding: utf-8 -*-
...
CPM
 ...
 chmod +x /etc/VPS-MIKU/protocols/lacasitamx.py &>/dev/nulll
