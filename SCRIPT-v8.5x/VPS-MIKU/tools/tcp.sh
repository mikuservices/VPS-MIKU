...
remove_all() {
	...
	echo -e "\e[1;31m DESINSTALADO BBR Accelerator\e[0m"
}
...
start_menu() {
	...
	msg -bar
	msg -tit
	echo -e " TCP Acceleration (BBR/Plus) ${Red_font_prefix}By @mikuservices${Font_color_suffix}
 $(msg -bar)
  ${Green_font_prefix}[ 1 ]${Font_color_suffix} Acelerar VPS con BBR ${amarillo}(recommended)${final}
  ${Green_font_prefix}[ 2 ]${Font_color_suffix} Acelerar VPS con BBRplus
  ${Green_font_prefix}[ 3 ]${Font_color_suffix} Detener la aceleracion BBR VPS
  ${Green_font_prefix}[ 0 ]${Font_color_suffix} Salir del script" && msg -bar

	run_status=$(grep "net.ipv4.tcp_congestion_control" /etc/sysctl.conf | awk -F "=" '{print $2}')
	if [[ ${run_status} ]]; then
		echo -e " Estado actual: ${Green_font_prefix}installedinstalled\n${Font_color_suffix} ${_font_prefix}BBR Comenzó exitosamente${Font_color_suffix} Kernel Acelerado, ${amarillo}${run_status}${Font_color_suffix}"
	else
		echo -e " Estado actual: ${Green_font_prefix}Not installed\n${Font_color_suffix} Kernel Acelerado: ${Red_font_prefix}Por favor,instale el Acelerador primero.${Font_color_suffix}"
	fi
	msg -bar
	read -p "$(echo -e "\e[31m► ${bla}Elige una opcion [0-3]:${amarillo}") " num
	case "$num" in
	0) ;;
	1) startbbr ;;
	2) startbbrplus ;;
	...
	*)
		clear
		echo -e "${Error}: Por favor inserta un numero correcto [0-3]"
		sleep 1s
		start_menu
		;;
	esac
