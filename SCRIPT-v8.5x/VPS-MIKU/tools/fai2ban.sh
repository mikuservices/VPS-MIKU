#!/bin/bash
 ...
 msg -bar
 msg -tit
 echo -e "\e[93m         --   Fail2ban Protecion v0.11.2 -- "
 ...
 if [[ ! -z "$pid_fail" ]]; then
  ...
  msg -bar
   while [[ -z ${logxyz} || ${logxyz} != @(1|2) ]]; do
    echo -ne "\033[1;37m$(fun_trans "Elige una opcion"): " && read logxyz
    tput cuu1 && tput dl1
   done
  case ${logxyz} in
 ...
 echo -e "${cor[5]}        Deseas instalar Fail2ban?"
 msg -bar
   while [[ -z ${fail2ban} || ${fail2ban} != @(s|S|n|N|y|Y) ]]; do
    echo -ne "\033[1;37m$(fun_trans "Elige una opcion") [S/N]: " && read fail2ban
    tput cuu1 && tput dl1
   done
