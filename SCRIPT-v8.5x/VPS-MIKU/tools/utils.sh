...
#INSTALA
msg -ama "$(fun_trans "TCP Speed ​​No esta activo, deseas activarlo ahora")?"
msg -bar
...
#REMOVE
msg -ama "$(fun_trans "TCP Speed ​ya esta activo, deseas desactivarlo ahora")?"
msg -bar
while [[ ${resposta} != @(s|S|n|N|y|Y) ]]; do
read -p " [Y/N]: " -e -i s resposta
tput cuu1 && tput dl1
...
echo -e " $(fun_trans "LOCAL MX DATE APLICADO!")"
}
resetiptables () {
...
echo -e "iptables reiniciado con exito"
}
packobs () {
msg -ama "Buscando paquetes obsoletos"
dpkg -l | grep -i ^rc
msg -ama "Limpiando y optimizando la VPS"
dpkg -l |grep -i ^rc | cut -d " " -f 3 | xargs dpkg --purge
msg -ama "Wipe obsolete packages complete | Limpieza completa "
}
...
msg -bar
msg -tit
msg -ama "                OPTMIMIZADORES BASICOS "
msg -bar
...
echo -ne "$(msg -bar)\n\033[1;32m [0] > " && msg -bra "\e[97m\033[1;41m REGRESAR \033[1;37m"
msg -bar
