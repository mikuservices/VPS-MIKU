#!/bin/bash
 ...
 Ban_BT(){
 	check_BT
 	[[ ! -z ${BT_KEY_WORDS} ]] && echo -e "${Error} Bloqueadas Torrent and Keywords, no necesitas realizar la accion !" && msg -bar2 && exit 0
 	s="A"
 	Set_BT
 	View_ALL
 ...
 Ban_SPAM(){
 	check_SPAM
 	[[ ! -z ${SPAM_PORT} ]] && echo -e "${Error} Blocked SPAM port detectado, no necesitas realizar la accion !" && msg -bar2 && exit 0
 	s="A"
 	Set_SPAM
 	View_ALL
 	echo -e "${Info} SPAM Ports Bloqueados !"
 	msg -bar2
 }
 Ban_ALL(){
 	check_BT
 	...
 	if [[ -z ${BT_KEY_WORDS} ]]; then
 		if [[ -z ${SPAM_PORT} ]]; then
 			Set_ALL
 			View_ALL
 			echo -e "${Info} Bloqueados Torrent, Keywords y SPAM Ports !"
 			msg -bar2
 		else
 			Set_BT
 			View_ALL
 			echo -e "${Info} Bloqueados Torrent y Keywords !"
 		fi
 	else
 		if [[ -z ${SPAM_PORT} ]]; then
 			Set_SPAM
 			View_ALL
 			echo -e "${Info} SPAM port (spam) prohibido !"
 		else
 			echo -e "${Error} Bloqueados Torrent, Keywords and SPAM Ports,\nno necesitas realizar la accion !" && msg -bar2 && exit 0
 		fi
 	fi
 }
 UnBan_BT(){
 	check_BT
 	[[ -z ${BT_KEY_WORDS} ]] && echo -e "${Error} Torrent and Keywords no bloqueados, revisalo !"&& msg -bar2 && exit 0
 	s="D"
 	Set_BT
 	View_ALL
 	echo -e "${Info} Desbloqueado Torrent y Keywords !"
 	msg -bar2
 }
 UnBan_SPAM(){
 	check_SPAM
 	[[ -z ${SPAM_PORT} ]] && echo -e "${Error} SPAM port no bloqueado, revisalo !" && msg -bar2 && exit 0
 	s="D"
 	Set_SPAM
 	View_ALL
 	echo -e "${Info} SPAM Ports Desbloqueado !"
 	msg -bar2
 }
 UnBan_ALL(){
 	check_BT
 	...
 	if [[ ! -z ${BT_KEY_WORDS} ]]; then
 		if [[ ! -z ${SPAM_PORT} ]]; then
 			Set_ALL
 			View_ALL
 			echo -e "${Info} Torrent, Keywords y SPAM Ports Desbloqueados !"
 			msg -bar2
 		else
 			Set_BT
 			View_ALL
 			echo -e "${Info} Torrent, Keywords Desbloqueados !"
 			msg -bar2
 		fi
 	else
 		if [[ ! -z ${SPAM_PORT} ]]; then
 			Set_SPAM
 			View_ALL
 			echo -e "${Info} SPAM Ports Desbloqueados !"
 			msg -bar2
 		else
 			echo -e "${Error} Torrent no detectado, Keywords and SPAM Ports Bloqueados, revisa !" && msg -bar2 && exit 0
 		fi
 	fi
 }
 ENTER_Ban_KEY_WORDS_type(){
  ...
  1. Manual (only unique keywords are supported)
  
  2. Local (supports batch reading of keywords, one keyword per line)
  
  3. Network address (supports batch reading of keywords, one keyword per line)" && echo
 ...
 ENTER_Ban_PORT(){
  ...
  -Multiport: 25, 26, 465, 587 (multiple ports separados por comas)
 
  -Continuous port segment: 25:587 (todos los puertos entre 25-587)" && echo
 	fi
 ...
 ENTER_Ban_KEY_WORDS(){
     msg -bar2
 	echo -e "introduce las keyboards a bloquear \n(domain name etc, only supports single keyword)"
 	if [[ ${Type_1} != "ban_1" ]]; then
 	echo ""
 	echo -e "${Green_font_prefix}======== Example Description ========${Font_color_suffix}
 	
 -Keywords: youtube, bloquean todos los dominios que incluyan la keyword youtube.
  
 -Keywords: youtube.com, bloquea todos los dominios (pan-domain name mask) que contengan youtube.com.
 
 -Keywords: www.youtube.com, bloquea todos los dominios (subdomain mask) que contengan www.youtube.com.
 ...
 msg -bar
 echo  -e "$(msg -tit) " 
 echo -e "  VPS•MIKU firewall panel By @mikuservices [ENGLISH] ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}"
 msg -bar2
 ...
 case "$num" in
 	...
 	;;
 	*)
 	echo "Por favor introduce el numero correcto [0-12]"
 	;;
 esac 