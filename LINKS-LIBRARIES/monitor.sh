...
IDB=`less /etc/VPS-MIKU/controller/IDT.log` > /dev/null 2>&1
IDB1=`echo $IDB` > /dev/null 2>&1
KEY="862633455:AAEgkSywlAHQQOMXzGHJ13gctV6wO1hm25Y"
URL="https://api.telegram.org/bot$KEY/sendMessage"
MSG="⚠️ _VPS AVISO:_ *$NOM1* ⚠️
❗️ _Protocolo_ *[ $1 ]* _con problemas_ ❗️ 
...
<head>
  ...
  <title>VPS-MIKU Service Monitor</title>
...
ureset_python () {
for port in $(cat /etc/VPS-MIKU/PortPD.log| grep -v "nobody" |cut -d' ' -f1)
do
PIDVRF3="$(ps aux|grep pydic-"$port" |grep -v grep|awk '{print $2}')"
if [[ -z $PIDVRF3 ]]; then
screen -dmS pydic-"$port" python /etc/VPS-MIKU/protocolos/PDirect.py "$port"
NOM=`less /etc/VPS-MIKU/controller/nombre.log` > /dev/null 2>&1
NOM1=`echo $NOM` > /dev/null 2>&1
IDB=`less /etc/VPS-MIKU/controller/IDT.log` > /dev/null 2>&1
IDB1=`echo $IDB` > /dev/null 2>&1
...
#SERVICE PY+SSL
ureset_pyssl () {
for port in $(cat /etc/VPS-MIKU/PySSL.log| grep -v "nobody" |cut -d' ' -f1)
do
PIDVRF3="$(ps aux|grep pyssl-"$port" |grep -v grep|awk '{print $2}')"
if [[ -z $PIDVRF3 ]]; then
screen -dmS pyssl-"$port" python /etc/VPS-MIKU/protocolos/python.py "$port"
NOM=`less /etc/VPS-MIKU/controller/nombre.log` > /dev/null 2>&1
NOM1=`echo $NOM` > /dev/null 2>&1
IDB=`less /etc/VPS-MIKU/controller/IDT.log` > /dev/null 2>&1
IDB1=`echo $IDB` > /dev/null 2>&1
